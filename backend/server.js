// npm init, to start, npm run dev (dev nodemon server package.json)
...
// put in env variables if deploy
...
app.use("/newExpense", require("./routes/newExpense"));

// put this after all the other routes to handle anything (*) not found
// status() sets a HTTP status on the response as a JS object to the client
app.all("*", (req, res) => {
  // 404 not found
  res.status(404);
